...
namespace Dynastream.Fit
{
  ...
  public class Mesg : MessageBase
  {
    ...
    #region Constructors
    public Mesg(Mesg mesg) : base(mesg)
    {
      if (mesg == null)
      {
        Name = "unknown";
      ...
      foreach (var kvp in mesg.DeveloperFields)
      {
        DeveloperField fld = kvp.Value;
    ...
    public Mesg(string name, ushort num) : base((Mesg)null)
    {
      Name = name;
      Num = num;
    }
    ...
    public bool HasField(byte fieldNum) => FieldsByNum.ContainsKey(fieldNum);
    ...
    private void AddCopy(Field field, int index = -1) => Add(new Field(field), index);
    private void Add(Field field, int index = -1)
    {
      FieldsByNum[field.Num] = field;
      FieldsByName[field.Name] = field;
    ...
    public int GetNumFields() => FieldsByNum.Count;
    ...
    public Field GetField(byte fieldNum) => FieldsByNum.TryGetValue(fieldNum, out Field field) ? field : null;
    ...
    {
      if (FieldsByName.TryGetValue(fieldName, out Field field))
      {
        return field;
      }
      ...
      foreach (var kvp in FieldsByNum)
      {
        ...
        foreach (var kvp2 in f.Subfields)
        {
          Subfield subfield = kvp2.Value;
    ...
    public string GetActiveSubFieldName(byte fieldNum)
    {
      ...
      foreach (var kvp in testField.Subfields)
      {
    ...
    {
      int index = Fields.Values.ToList().IndexOf(field);
      if (index > -1)
      {
        Fields.Remove(index);
      }
    ...
    {
      foreach (var kvp in FieldsByNum.Where(kvp => kvp.Value.IsExpandedField))
      {
        Field field = kvp.Value;
        RemoveField(field);
      }
    ...
    public void ExpandComponents(Accumulator accumulator)
    {
      ...
      foreach (var kvp in fieldsByNum)
      {
        ...
        Dictionary<int, FieldComponent> components = null;
        // Determine the active subfield
        ushort activeSubfield = GetActiveSubFieldIndex(field.Num);
        ...
        {
          components = field.GetSubfield(activeSubfield).Components;
        }

        // Traverse the component list
        int offset = 0;
        foreach (FieldComponentExpansion f in ExpandComponentsInList(components.Values.ToList(), field, offset, accumulator))
        {
          //Add the new field
          Add(f.GetField());
          //update offset
