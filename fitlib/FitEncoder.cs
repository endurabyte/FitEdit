...
using DateTime = fitsharplib.Fit.DateTime;
using Encoder = fitsharplib.Fit.Encoder;
...
namespace fitsharp
{
    public class FitEncoder
    {
        ...
        static void EncodeActivityFile()
        {
            ...
            fileIdMesg.SetType(fitsharplib.Fit.File.Activity);
        ...
        static void EncodeSettingsFile()
        {
            ...
            fileIdMesg.SetType(fitsharplib.Fit.File.Settings);
            fileIdMesg.SetManufacturer(Manufacturer.Garmin);  // Types defined in the profile are available
            fileIdMesg.SetProduct(3113);
            fileIdMesg.SetSerialNumber(3998947172);
        ...
        static void EncodeMonitoringFile()
        {
            ...
            FileIdMesg fileIdMesg = new FileIdMesg(); // Every FIT file MUST contain a 'File ID' message as the first message
            fileIdMesg.SetSerialNumber(3998947172);
            fileIdMesg.SetTimeCreated(new fitsharplib.Fit.DateTime(systemTimeNow));
            fileIdMesg.SetManufacturer(Manufacturer.Garmin);
            fileIdMesg.SetProduct(1752);
            fileIdMesg.SetNumber(0);
            fileIdMesg.SetType(fitsharplib.Fit.File.MonitoringB); // See the 'FIT FIle Types Description' document for more information about this file type.
            encoder.Write(fileIdMesg); // Write the 'File ID Message'

            DeviceInfoMesg deviceInfoMesg = new DeviceInfoMesg();
            deviceInfoMesg.SetTimestamp(new fitsharplib.Fit.DateTime(systemTimeNow));
            deviceInfoMesg.SetSerialNumber(5180063);
            deviceInfoMesg.SetManufacturer(Manufacturer.Garmin);
            deviceInfoMesg.SetBatteryStatus(fitsharplib.Fit.BatteryStatus.Good);
            ...
            for (int i = 0; i < 4; i++) // Each of these loops represent a quarter of a day
            {
                ...
                {
                    monitoringMesg.SetTimestamp(new fitsharplib.Fit.DateTime(systemTimeNow));
                    monitoringMesg.SetActivityType(fitsharplib.Fit.ActivityType.Walking); // Setting this to walking will cause Cycles to be interpretted as steps.
                ...
                deviceInfoMesg.SetTimestamp(new fitsharplib.Fit.DateTime(systemTimeNow));
                deviceInfoMesg.SetBatteryStatus(fitsharplib.Fit.BatteryStatus.Good); // Report the battery status every quarter day
                encoder.Write(deviceInfoMesg);
            }
